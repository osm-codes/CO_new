...
(
  ...
  rpad(cell_bits, 37, '0000000000000000000000000000000000000') AS sufix_bits, q.isolabel_ext, cell, ordered_cover,
  ...
  LEFT JOIN LATERAL
  (
    SELECT (CASE WHEN length(p.cell)>1 THEN str_ggeohash_decode_box2(substring(p.cell_bits from 6),32,bbox) ELSE bbox END) AS bbox
